...
const handlers = {
                                ...
                                if (response2.data.similars.length > 0) {
                                    if (response2.data.similars.length > 5) {
                                        ...
                                        firstFives.forEach(function(item, index, array) {
                                            ...
                                            speechOutput += '<break time="1s"/> ' + (vm2.attributes.choice === 'film') ? (item.movie_title + "<break time='1s'/>") : (item.show_title + "<break time='1s'/>")
                                        })
                                    } else {
    ...
    'AMAZON.HelpIntent': function() {
        this.response.speak('La skill Watch it est faites pour vous aider à trouver des informations à propos de films et de séries.' + 'Dans un premier temps dites film si vous voulez des informations sur un film ou série si vous voulez des informations sur une série <break time="1s"/> Ensuite,' + 'Voilà quelques exemples de phrases que vous pouvez dire pour votre recherche : ' +
            Helpers.responseHelper(yearUtterance) + '<break time="1s"/>' + Helpers.responseHelper(SynopsisUtterance) + '<break time="1s"/>' + Helpers.responseHelper(markUtterance)
        ).listen('Voilà quelques exemples de phrases que vous pouvez dire pour aller plus loin dans votre recherche : ' + Helpers.responseHelper(yearUtterance) + '<break time="1s"/>' + Helpers.responseHelper(charactersUtterance))
